"""
DKR Standalone launch — RMF'den bağımsız trafik yönetimi.

Simülasyon (Gazebo + RViz) ayrı terminal'de çalışır:
  ros2 launch rmf_demos_gz warehouse_starter_standalone.launch.xml

Bu launch dosyası:
  1. DKR Standalone Traffic Manager (görev atama + rota planlama + kaynak rezervasyonu)
  2. Metric Logger (deney metrikleri)
  3. DKR Task Dispatcher (senaryo görevlerini DKR'ye gönderir)

Statik nav graph marker'ları sim launch'ta başlar (reservation_viz → /dkr_markers).
Aktif robot rotaları bu launch ile reservation_viz üzerinden /dkr_route_markers'a publish edilir.
...
def generate_launch_description():
    ...
    dkr_node = Node(
        ...
        executable="dkr_standalone_manager",
    ...
    task_dispatcher_node = Node(
        package="dkr_controller",
        executable="task_dispatcher_dkr",
        name="task_dispatcher_dkr",
        output="screen",
        parameters=[{
